...
class JHMDBSentencesDataset(Dataset):
    ...
    """
    def __init__(self, subset_type: str = 'test', dataset_path: str = './datasets/jhmdb_sentences', window_size=8,
        ...
        if subset_type == 'test' and not path.exists(dataset_coco_gt_format_path):
            if (distributed and dist.get_rank() == 0) or not distributed:
                output_path = f'./datasets/jhmdb_sentences/jhmdb_sentences_gt_annotations_in_coco_format.json'
                create_jhmdb_sentences_ground_truth_annotations(self.samples_metadata, output_path, **kwargs)
            if distributed:
                dist.barrier()
