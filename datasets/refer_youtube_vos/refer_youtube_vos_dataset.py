...
class ReferYouTubeVOSDataset(Dataset):
    ...
    """
    def __init__(self, subset_type: str = 'train', dataset_path: str = './datasets/refer_youtube_vos', window_size=12,
    ...
    def generate_samples_metadata(self, dataset_path, subset_type, window_size, distributed):
        ...
        if path.exists(metadata_file_path):
            ...
            if subset_type == 'train':
                ...
                subset_expressions_by_video = {k: subset_expressions_by_video[k] for k in valid_plus_test_videos}
                assert len(subset_expressions_by_video) == 507, 'error: incorrect number of validation expressions'
    ...
    def __getitem__(self, idx):
        ...
        if self.subset_type == 'train':
            with open(path.join('./datasets/refer_youtube_vos/train', 'meta.json'), 'r') as f:
