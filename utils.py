...
def flatten_temporal_batch_dims(outputs, targets):
    for k in outputs.keys():
        if k == 'pred_logit' or k == 'text_sentence_feature' or k == 'pred_cls' or k == 'high_features' or k == 'low_features' or k == 'memory_outputs':
            continue
        if isinstance(outputs[k], torch.Tensor):
            outputs[k] = outputs[k].flatten(0, 1)
        else:  # list
            if all(hasattr(step_t, '__iter__') for step_t in outputs[k]):
