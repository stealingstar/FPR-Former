...
class HungarianMatcher(nn.Module):
    ...
    def forward(self, outputs, targets, text_refer):
        ...
        # Check if various losses need to be computed
        # Compute the soft-tokens cost:
        if self.cost_con > 0 and text_refer and 'pred_logit' in outputs and 'text_sentence_feature' in outputs:
            cost_con = text_refer_cost(outputs, targets)
        else:
            cost_con = 0
            
        if self.cost_cls > 0 and 'pred_cls' in outputs:
            cost_cls = compute_label_cost(outputs, targets, self.num_classes)
        else:
            cost_cls = 0
            
        # Check if box related losses need to be computed
        if self.cost_box > 0 and 'pred_boxes' in outputs:
            output_boxes = outputs['pred_boxes']
            cost_box = costs_box(output_boxes, tgt_boxes)
        else:
            cost_box = 0
            
        if self.cost_giou > 0 and 'pred_boxes' in outputs:
            output_boxes = outputs['pred_boxes']
...
def text_refer_cost(outputs, targets):
    ...
    if 'pred_logit' not in outputs or 'text_sentence_feature' not in outputs:
        # If necessary keys do not exist, return zero cost
        device = outputs['pred_masks'].device
        batch_size = outputs['pred_masks'].shape[1]
        num_queries = outputs['pred_masks'].shape[2]
        return torch.zeros((batch_size * num_queries, 1), device=device)
